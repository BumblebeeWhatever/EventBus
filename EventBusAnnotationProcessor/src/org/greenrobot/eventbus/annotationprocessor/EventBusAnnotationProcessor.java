...
@SupportedOptions(value = {EventBusAnnotationProcessor.OPTION_EVENT_BUS_INDEX, EventBusAnnotationProcessor.OPTION_VERBOSE})
public class EventBusAnnotationProcessor extends AbstractProcessor {
    ...
    /**
     * {@see https://blog.csdn.net/u010405231/article/details/52210401}
     *
     * @param annotations 需处理的注解
     * @param env         当前的运行环境
     * @return
     */
    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment env) {
        ...
        try {
            String index = processingEnv.getOptions().get(OPTION_EVENT_BUS_INDEX);//FIXME what for?
            if (index == null) {
    ...
    /**
     * Element代表package包、class类、method方法
     * <p>
     * Element子类包含如下:
     * 1、PackageElement:表示一个包程序元素，提供对有关包极其成员的信息访问。对应@Target(ElementType.PACKAGE)
     * 2、TypeElement:表示一个类或接口程序元素。提供对有关类型极其成员的信息访问。对应@Target(ElementType.TYPE),(注意：枚举类型是一种类，而注解类型是一种接口)
     * 3、TypeParameterElement:表示一般类、接口、方法或构造方法元素的类型参数。对应@Target(ElementType.PARAMETER)
     * 4、VariableElement:表示一个字段、enum常量、方法或构造方法参数、局部变量或异常参数。对应@Target(ElementType.LOCAL_VARIABLE)
     * 5、ExecutableElement:表示某个类或接口的方法、构造方法或初始化程序（静态或实例），包括注释类型元素。对应@Target(ElementType.METHOD) @Target(ElementType.CONSTRUCTOR)
     **/
    ...
    private void collectSubscribers(Set<? extends TypeElement> annotations, RoundEnvironment env, Messager messager) {
        for (TypeElement annotation : annotations) {
            ...
            for (Element element : elements) {
                if (element instanceof ExecutableElement) {//ExecutableElement是指方法
                    ExecutableElement method = (ExecutableElement) element;
                    if (checkHasNoErrors(method, messager)) {
                        TypeElement classElement = (TypeElement) method.getEnclosingElement();//获取方法所在的类
    ...
    private void createInfoIndexFile(String index) {
        ...
        try {
            ...
            writer.write("    //key为订阅者Class，value为订阅者的信息\n");
    ...
    /**
     * typeElement is visible when typeElement is public or it's class.getName().equals(myPackage)
     *
     * @param myPackage
     * @param typeElement
     * @return if typeElement is visible
     */
